package cs.rit.edu.GUI;import java.sql.Date;
...
public class ToolInterface extends Application{
    ...
    HBox collectionList, ownedList;    @Override
    public void stop() {
        conn.closeConn();
    }   @Override
    public void start(Stage stage) throws Exception {
        TextField username = new TextField();
        username.setPromptText("Username");      Button loginBtn = new Button();
        loginBtn.setText("Login");
        loginBtn.setOnAction(new EventHandler<ActionEvent>() {            @Override
        public void handle(ActionEvent event) {
            String usernameInput = username.getText();                System.out.println(Integer.parseInt(usernameInput));                appUser = new User(Integer.parseInt(usernameInput), conn);                refreshToolCollection(appUser.getToolCollection());
        ...
        usernameGroup.getChildren().add(username);        HBox login = new HBox();
        login.setAlignment(Pos.CENTER_LEFT);
        login.getChildren().add(usernameGroup);
        login.getChildren().add(loginBtn);        //Collection in the center
        HBox toolLists = new HBox();        collectionList = new HBox();
        ownedList = new HBox();        createCollections();        toolLists.getChildren().add(collectionList);
        toolLists.getChildren().add(new Separator());
        toolLists.getChildren().add(ownedList);        VBox toolCreation = toolCreationMenu();        lendPane = new VBox();
        Label lendPaneLabel = new Label("Lending");
        lendPane.getChildren().add(lendPaneLabel);        BorderPane pane = new BorderPane();
        pane.setTop(login);
        pane.setCenter(toolLists);
        pane.setLeft(toolCreation);
        pane.setRight(lendPane);        Scene scene = new Scene(pane, 300, 250);        stage.setTitle("Tool Interface");
        stage.setScene(scene);
        stage.show();
    }   public void createCollections() {      //Creating columns for the collection
        collectionToolName = new VBox();
        collectionToolName.getChildren().add(new Label("Tool Names"));
        collectionPurchaseDate = new VBox();
        collectionPurchaseDate.getChildren().add(new Label("Purchase Dates"));        //Creating columns for the owned tools section
        ...
        ownedLendable.getChildren().add(new Label("Lendability"));        collectionList.getChildren().add(collectionToolName);
        collectionList.getChildren().add(new Separator());
        collectionList.getChildren().add(collectionPurchaseDate);        ownedList.getChildren().add(ownedToolName);
    ...
    public void refreshToolCollection(List<Tool> tools) {
        for (Tool tool: tools) {
            ...
            Label purchaseDate = new Label(tool.getPurchaseDate().toString());         collectionToolName.getChildren().add(toolName);
        ...
        }        HashMap<Integer, String> allUsers = conn.fetchAllUsers();        ArrayList<String> usersNames = new ArrayList<>();
        ...
        ComboBox<String> usersDrop = new ComboBox<String>(FXCollections.observableArrayList(usersNames));        lendPane.getChildren().add(toolsDrop);
        ...
        TextField toolNameEntry = new TextField();        HBox toolName = new HBox();
        toolName.getChildren().add(toolNameLabel);
        toolName.getChildren().add(toolNameEntry);        Label toolPurchaseLabel = new Label("Date Purchased: ");
        DatePicker toolPurchaseDate = new DatePicker();        HBox toolPurchase = new HBox();
        toolPurchase.getChildren().add(toolPurchaseLabel);
        toolPurchase.getChildren().add(toolPurchaseDate);        Label lendableToolLabel = new Label("Is Tool Lendable: ");
        CheckBox lendableToolCheck = new CheckBox();        HBox lendableTool = new HBox();
        lendableTool.getChildren().add(lendableToolLabel);
        lendableTool.getChildren().add(lendableToolCheck);        //Tool Types Checkboxes
        HBox listOfTypes = new HBox();
        VBox toolTypeName = new VBox();
        VBox checkBoxes = new VBox();        listOfTypes.getChildren().add(toolTypeName);
        listOfTypes.getChildren().add(checkBoxes);        HashMap<Integer, String> toolTypes = conn.fetchAllToolTypes();
        List<CheckBox> selectedToolTypes = new ArrayList<>();        for(String toolType: toolTypes.values()) {
            Label toolTypeLabel = new Label(toolType);
            toolTypeName.getChildren().add(toolTypeLabel);           CheckBox checkBox = new CheckBox();
            checkBoxes.getChildren().add(checkBox);
            selectedToolTypes.add(checkBox);
        }        Button createTool = new Button();
        createTool.setText("Create Tool");
        createTool.setOnAction(new EventHandler<ActionEvent>() {            @Override
        public void handle(ActionEvent event) {
...
//                appUser.addToOwned(newTool) ;
        ...
        toolCreation.getChildren().add(createTool);        return toolCreation;
    }   /**
    ...
    public void refreshToolsOwned(List<Tool> tools) {
        for (Tool tool: tools) {
            ...
            Label lendable = new Label((tool.isLendable() ? "True": "False"));         ownedToolName.getChildren().add(toolName);
        ...
        if(args.length > 1) {
            ...
            String password = args[1];            conn = new DBConn(username, password);         launch(args);
        }else {
